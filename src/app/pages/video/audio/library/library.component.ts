...
export class AudioLibraryComponent implements OnInit {
    ...
    showDetail(data: any) {
        this.http.get("mix/video/admin/audio-data/detail", { id: data.id }).then(ret => {
            ...
            this.form.body = { ...ret, cover: ret.cover_path };
            this.isVisible = true;
    ...
    save() {
        if (this.params.id) {
            this.form.action = "mix/video/admin/audio-data/update";
        } else {
            this.form.action = "mix/video/admin/audio-data/create";
        }
    ...
    remove(data: any) {
        this.modalService.confirm({
            nzTitle: "确定要删除该音频吗?",
            nzOnOk: () => {
                this.http.post("mix/video/admin/audio-data/delete", { id: data.id }).then(() => {
    ...
    cancel() {
        if (this.isUploadVideo) {
            this.notification.info("提示信息", "正在上传音频");
            return;
        }
        this.isVisible = false;
        this.disabledId = true;
    ...
    getVideoList() {
        ...
        this.http.get("mix/video/admin/audio-data/temp-audio-list", params).then(ret => {
