...
export class SalarySheetComponent implements OnInit {
    ...
    public async details(item) {
        if (item) {
            ...
            const data = await this.http.post('finance/salary/admin-city/staff-payroll/detail', params);
            ...
            if (data) {
                ...
                if(!data.payroll_id){ 
                    ...
                    this.recordUrl = 'finance/salary/admin-city/payroll-adjust-record/list?id=' + data.payroll_id
    ...
    public async save() {
        this.form.action = 'finance/salary/admin-city/staff-payroll/save';
        try {
