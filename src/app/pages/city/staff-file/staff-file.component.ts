...
export class StaffFileComponent implements OnInit {
    ...
    public listUrl = 'finance/salary/admin-city/staff-profile/list';
    ...
    constructor(
        ...
        this.activatedRoute.data.subscribe(async (value) => {
            this.group = value.group || '';
            const ret = await this.getGroup(this.group);
            this.roleManage = ret;
            this.listUrl = 'finance/salary/admin-city/staff-profile/list' + this.getGroupQueryStr();
        });
    ...
    protected getGroupQueryStr() {
        const params = this.makeGroupParams(this.roleManage);
    ...
    public async findUserInfo() {
        const data = await this.http.post('finance/salary/admin-city/staff-profile/search-staff', { mobile: this.mobile });
        if (data) {
    ...
    public async fileModalOk() {
        ...
        try {
            const ret = await this.http.post('finance/salary/admin-city/staff-profile/sync-staff-info', params);
    ...
    public async edit(item) {
        ...
        const data = await this.http.post('finance/salary/admin-city/staff-profile/detail', { id: item.id });
        if (data) {
    ...
    public async save() {
        try {
            this.form.action = 'finance/salary/admin-city/staff-profile/adjust';
    ...
    public detailsSelectChange(event) {
        if (event.index === 2) {
            ...
            // eslint-enable
            this.salaryAdjustmentListUrl = 'finance/salary/admin-city/adjust-record/list' + this.getGroupQueryStr() + '&id=' + this.userDetail.id;
            /*eslint-enable*/
        }
    }
    ...
    public backOut(item) {
        this.modalService.confirm({
            ...
            nzCancelText: '取消',
            nzOnOk: async () => {
                const data = await this.http.post('finance/salary/admin-city/adjust-record/revocation', { id: item.id });
                if (data === 'success') {
