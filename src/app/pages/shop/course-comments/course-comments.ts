...
export class HallCourseCommentsComponent implements OnInit {
    collection: any = {};
    ...
    getCommentDetail(data: any) {
        this.http.get("hall/admin-hall/comment/detail", { id: data.id }).then(ret => {
            if (!ret.comment_tags) {
                ret.comment_tags = [];
            }
    ...
    replyComment() {
        ...
        this.http.post("hall/admin-hall/comment/reply", params).then(() => {
