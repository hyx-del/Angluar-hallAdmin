...
@Component({
    selector: 'app-sales-performance-statistics',
    templateUrl: './sales-performance-statistics.html',
    styleUrls: ['./sales-performance-statistics.scss']
})
export class SalesPerformanceStatisticsComponent implements OnInit {
    collection: any = {};
    ...
    getDataList() {
        ...
        if (this.currentSale) {
            params.salesman_id = this.currentSale;
        }

        this.http.get("hall/admin-hall/sales-performance-statistics", params).then(ret => {
            let headers = [];
            if (ret && ret.type) {
